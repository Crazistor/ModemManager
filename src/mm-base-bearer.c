...
#define MAX_SECONDARY_MONITOR_ERRORS 6
                            ...
                            SecondaryConnectionContext *ctx)
{
    ...
    if (!MM_BASE_BEARER_GET_CLASS (self)->disconnect_secondary_finish (self, res, &error)) {
        mm_dbg ("Secondary connection context (#%u) disconnection failed: %s", ctx->idx, error->message);
        g_error_free (error);
    } else
        mm_dbg ("Secondary connection context (#%u) disconnected", ctx->idx);
                                        ...
                                        SecondaryConnectionContext *ctx)
{
    ...
    case MM_BEARER_CONNECTION_STATUS_DISCONNECTED:
        /* Need to reconnect? */
        mm_dbg ("secondary connection context (#%u) is disconnected", ctx->idx);
        g_assert (!error);
        ctx->step = SECONDARY_CONNECTION_CONTEXT_STEP_CONNECT;
        secondary_connection_context_step (ctx);
        return;
                         ...
                         SecondaryConnectionContext *ctx)
{
    ...
    if (!MM_BASE_BEARER_GET_CLASS (self)->connect_secondary_finish (self, res, &error)) {
        mm_dbg ("Couldn't connect secondary connection context (#%u): %s", ctx->idx, error->message);
    ...
    /* Handle cancellations detected after successful connection */
        ...
        mm_dbg ("Connected secondary connection context (#%u), but need to disconnect", ctx->idx);
        /* explicitly disconnect */
        ctx->step = SECONDARY_CONNECTION_CONTEXT_STEP_DISCONNECT;
    }
    else {
        mm_dbg ("Connected secondary connection context (#%u)", ctx->idx);
        ctx->step++;
    }
    g_clear_object (&ctx->connect_cancellable);
