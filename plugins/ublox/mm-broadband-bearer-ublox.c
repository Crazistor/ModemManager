               ...
               GTask        *task)
{
    ...
    mm_dbg ("Primary context (cid %u) statistics: rx %u bytes, tx %u bytes", cid, total_tx_bytes, total_rx_bytes);
    ...
    for (i = 0; i < n; i++) {
        ...
        mm_dbg ("Secondary context (cid %u) statistics: rx %u bytes, tx %u bytes", cid, tx_bytes, rx_bytes);

        total_tx_bytes += tx_bytes;
        total_rx_bytes += rx_bytes;
    }

    mm_dbg ("Total context statistics: rx %u bytes, tx %u bytes", total_tx_bytes, total_rx_bytes);
